...
class testWorker(QObject):
    ...
    def test(self):
        global train_data, Y, pose_predict, pose_train
        ...
        if len(train_data) == 0:
            ...
            for i in range(train_reps):
                ...
                if type(t) == int:
                    t = np.linspace(1, len(smoothed.CH1)/500, num=len(smoothed.CH1))
                else:
                    t = np.linspace(bgn, bgn+len(smoothed.CH1)/500, num=len(smoothed.CH1))
                ...
                y=get_one_hot(smoothed.Pose.astype(int),3)
                ...
                pose_predict = Y[:,0]*0 + Y[:,1]*1 + Y[:,2]*2
                pose_train = y[:,0]*0 + y[:,1]*1 +y[:,2]*2
                
                plt.plot(t, pose_train, color="blue")
                plt.scatter(t, pose_predict, color = "orange",s = 6)
                plt.legend(["Actual","Estimated"],loc = 'upper left')
                plt.yticks([0,1,2],['Rest','Open','Close'])
                plt.xlabel("Time [s]")
                title = "Results from Testing Deep Learning Model"
                plt.title(title)
                
                poseCount = np.count_nonzero(Y,axis=0)
                print(poseCount)
                pose = np.argmax(poseCount)
            ...
            t = np.linspace(1, len(train_data.CH1)/500, num=len(train_data.CH1))
...
class MainWin(QMainWindow):
    ...
    def __init__(self):
        ...
        self.instruct.setGeometry(QRect(900, 0, 300, 800))
    ...
    def _createActions(self):
        ...
        self.promptAction = QAction("&New Prompt...", self)
    ...
    def _createMenuBar(self):
        ...
        fileMenu.addAction(self.promptAction)
    ...
    def _connectActions(self):
        ...
        self.promptAction.triggered.connect(self.newPrompt)
    ...
    def loadData(self):
        ...
        self.topdisp.setText(f"{fname} successfully uploaded!")
    
    def loadModel(self):
        global model
        folder = str(QFileDialog.getExistingDirectory(self, "Select Directory"))
        model = keras.models.load_model(folder)
        self.topdisp.setText(f"{folder} succuessfully uploaded!")
        ...
        while True:
            ...
            if all([isinstance(item, int) for item in prompt]):
                if all([item >= 0 for item in prompt]) and all([item <= 4 for item in prompt]):
                    ...
                    self.topdisp.setText("Integers should be between 0 and 4")
                    continue
            else:
                self.topdisp.setText("Create input as integers seperated by space")
                continue
    ...
    def testData(self):
        ...
        if model == None:
            self.topdisp.setText("CANNOT TEST NON-EXISTENT MODEL")
        else:
            self.timeThread.start()
            self.collect.setEnabled(False)
            self.train.setEnabled(False)
    ...
    def trainData(self):
        ...
        if  len(train_data) == 0:
            self.topdisp.setText("CANNOT TRAIN DATA ARRAY OF LENGTH 0")
        else:
